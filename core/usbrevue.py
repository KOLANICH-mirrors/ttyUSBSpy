...
USBMON_TRANSFER_TYPE = dict(isochronous=0, interrupt=1, control=2, bulk=3)
...
class PackedFields(object):
	...
	def __setattr__(self, attr, val):
		...
		if attr in self.format_table:
			...
			if self.update_parent is not None:
...
class Packet(PackedFields):
	...
	def data(self):
		...
		return self.cache("data", lambda a: list(self.unpacket(a, self.datalen)))
	...
	def setup(self):
		...
		if self.is_setup_packet:
			return self.cache("setup", lambda a: SetupField(self.unpacket(a)[0], partial(_update_setup, self)))
	...
	def print_fecha_dato(self):
		global timeMs
		nowMs = self.ts_sec * 1000 + self.ts_usec / 1000
		if timeMs != 0:
			oldMs = timeMs
			deltaMs = nowMs - oldMs
	...
	def get_dato_serie(self, b):
		...
		b.nowMs = self.ts_sec * 1000 + self.ts_usec / 1000
		b.xfer_type = self.xfer_type
		b.epnum = self.epnum
		b.direc = "{0}{1}".format("ZICB"[self.xfer_type], "oi"[self.epnum >> 7])
		b.devnum = self.devnum
		b.setup = "0"
		if b.direc == "Co" or b.direc == "Ci":
			b.data = "|"
			if self.is_setup_packet:
...
SETUP_FIELD_FORMAT = dict(bmRequestType=("<B", 0), bRequest=("<B", 1), wValue=("<H", 2), wIndex=("<H", 4), wLength=("<H", 6))
...
REQUEST_TYPE_MASK = dict(direction=0b10000000, type_=0b01100000, recipient=0b00011111)
...
class SetupField(PackedFields):
	...
	def bmRequestTypeDirection(self, val):
		self.bmRequestType = apply_mask(REQUEST_TYPE_MASK["direction"], self.bmRequestType, REQUEST_TYPE_DIRECTION[val])
	...
	def bmRequestTypeType(self, val):
		self.bmRequestType = apply_mask(REQUEST_TYPE_MASK["type_"], self.bmRequestType, REQUEST_TYPE_TYPE[val])
	...
	def bmRequestTypeRecipient(self, val):
		self.bmRequestType = apply_mask(REQUEST_TYPE_MASK["recipient"], self.bmRequestType, REQUEST_TYPE_RECIPIENT[val])
	...
	def data_to_str(self):
		...
		"""
		return "%02X %02X %02X%02X %02X%02X %02X%02X" % unpack("<8B", self.datapack.tostring())  # yuck

	def fields_to_str(self):
		"""Verbose but single-line string representation of setup data.
		"""
		s = "bmRequestType: %s, %s, %s (%s)" % (self.bmRequestTypeType, self.bmRequestTypeDirection, self.bmRequestTypeRecipient, bin(self.bmRequestType))
...
if __name__ == "__main__":
	...
	pcap = pcapy.open_offline("../../fagor0")
	# out = pcap.dump_open('-')
